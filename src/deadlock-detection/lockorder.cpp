    ...
    const uint64_t &tid)
{
    ...
    {
        LogPrintf("either %s or %s was passed by a pointer, the lock names might not be accurate \n",
    ...
    const std::vector<LockStackEntry> &heldLocks)
{
    std::lock_guard<std::mutex> lock(lot_mutex);
    // build the new key
    std::string new_key = locklocation.GetMutexName() + " on " + locklocation.GetFileName() + ":" + std::to_string(locklocation.GetLineNumber());
    ...
    for (auto &heldLock : heldLocks)
    {
        std::string held_key = heldLock.second.GetMutexName() + " on " + heldLock.second.GetFileName() + ":" + std::to_string(heldLock.second.GetLineNumber());
